...
/*   Updated: 2023/06/30 12:25:05 by jternero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	if (argc == 2)
	{
		...
		{
			free(game.map);
			return (ft_printf("ERROR2, INVALID MAP!!\n"));
		}
		...
		{
			ft_free(game.map);
			return (ft_printf("ERROR3, INVALID MAP!!\n"), -1);
		}
		ft_game_main(&game);
		ft_free(game.map);
	}
	else
		return (ft_printf("ERROR4, INVALID ARGUMENTS!!\n"));
}
